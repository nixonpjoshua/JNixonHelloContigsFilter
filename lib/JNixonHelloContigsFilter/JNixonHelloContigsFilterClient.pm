package JNixonHelloContigsFilter::JNixonHelloContigsFilterClient;
...
JNixonHelloContigsFilter::JNixonHelloContigsFilterClient
...
sub new
{
    ...
    my $self = {
	client => JNixonHelloContigsFilter::JNixonHelloContigsFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "JNixonHelloContigsFilter.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "JNixonHelloContigsFilter.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "JNixonHelloContigsFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for JNixonHelloContigsFilter::JNixonHelloContigsFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "JNixonHelloContigsFilter::JNixonHelloContigsFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package JNixonHelloContigsFilter::JNixonHelloContigsFilterClient::RpcClient;
